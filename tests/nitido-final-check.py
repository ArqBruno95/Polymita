...
if 'WireShelfOriginalDoc' not in sc.sticky:
    sc.sticky['WireShelfOriginalDoc'] = canvas.Document
...
assembly = System.Reflection.Assembly.LoadFrom(os.path.join(root, 'test-output', 'WireShelf.NitidoTest03.dll'))
runtime = assembly.GetType('WireShelf.ShelfRuntime')
...
canvas=Instances.ActiveCanvas
flags=BindingFlags.NonPublic|BindingFlags.Public|BindingFlags.Static
assembly=next(a for a in System.AppDomain.CurrentDomain.GetAssemblies() if a.GetName().Name=='WireShelf.NitidoTest03')
runtime=assembly.GetType('WireShelf.ShelfRuntime')
...
if canvas.Document==sc.sticky.get('WireShelfToolboxFixture'):
 ...
 print('Nítido labels and angular wire preview ready.')
