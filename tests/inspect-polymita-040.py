...
assembly=next(a for a in System.AppDomain.CurrentDomain.GetAssemblies() if a.GetName().Name=='Polymita.PolymitaTest02')
runtime=assembly.GetType('Polymita.ShelfRuntime')
box=runtime.GetField('toolbox',flags).GetValue(None)
with open(os.path.join(root,'test-output','polymita-ui.txt'),'w') as f:
 def walk(c,level):
